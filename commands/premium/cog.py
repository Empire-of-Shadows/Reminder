...
class PremiumCog(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        # Attached in attach_databases() before cogs load.
        self.premium_manager = getattr(bot, "premium_manager", None)
        if self.premium_manager is None:
            raise RuntimeError(
                "bot.premium_manager is not attached - attach_databases must run before cogs load"
            )
    ...
    async def status(self, interaction: discord.Interaction):
        ...
        if state.is_premium:
            ...
            embed.set_footer(text="Thank you for supporting Imperial Reminder!")
        else:
            embed.add_field(name="Status", value="Free Tier", inline=True)
            embed.description = (
                "Upgrade to Premium to unlock custom reminder messages, webhook "
                "delivery, and shorter reminder cooldowns on supported bump bots."
            )
        if limits:
